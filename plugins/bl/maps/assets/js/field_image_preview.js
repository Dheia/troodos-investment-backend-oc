...
function imageMapInit() {
    ...
    if (!imagePath) {
        imagePath = $(
            "#Form-relationImageViewForm-field-MediaItem-full_path-group .form-control"
        )
            .text()
            .trim();
    }
    if (imagePath) {
        ...
        img.onload = function() {
            ...
            var southWest = imarkersMap.unproject(
                [0, this.height],
                imarkersMap.getMaxZoom() - 1
            );
            var northEast = imarkersMap.unproject(
                [this.width, 0],
                imarkersMap.getMaxZoom() - 1
            );
...
$(function() {
    ...
    var fullPathEl = $(
        "#MapsController-create-RelationController-image-view"
    ).get(0);
